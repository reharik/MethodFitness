module.exports = function(
  ...
  return async continuationId => {
    ...
    logger.info(
      'subscription.isSubscribedToAll: ' + subscription.isSubscribedToAll,
    );

    let stream = rx.Observable.fromEvent(eventAppeared.emitter, 'event')
      .first(note => {
        return (
          mAndF.continuationId(note).getOrElse() === continuationId &&
          ef.parseData(note).getOrElse().initialEvent.metadata.streamType ===
            'command'
        );
      })
