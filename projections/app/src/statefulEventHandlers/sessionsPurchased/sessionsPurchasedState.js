module.exports = function(R, metaLogger) {
  return function sessionsPurchasedState(innerState) {
    const cleanUp = event => {
      ...
      purchaseIds.forEach(x => {
        if (
          ...
          innerState.purchases = innerState.purchases.filter(
            p => p.purchaseId !== x,
          );
        }
      });
    ...
    const getPurchase = purchaseId => {
      let purchase = innerState.purchases.find(
        x => x.purchaseId === purchaseId,
      );
      if (purchase) {
        const sessions = innerState.sessions.filter(
    ...
    const pastAppointmentUpdated = event => {
      ...
      event.clients.forEach(c => {
        let session = innerState.sessions.find(
          x => x.appointmentId === event.appointmentId && x.clientId === c,
        );
        if (
          session &&
