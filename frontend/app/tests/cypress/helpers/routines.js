module.exports = (cy, Cypress, moment) => {
  ...
  const _changeClients = (options) => {
    ...
    if (!!options.newClient && !!options.newClient2) {
      ...
      cy.dataId('clients-container', 'div')
        ...
        .each(item => cy.wrap(item).find('span.ant-select-selection__choice__remove')
          .click());
      cy.get('#clients').click();
      cy
        .get('.ant-select-dropdown-menu-item')
  ...
  const checkPayTrainer = (options) => {
    cy.log(`======================================================`);
    cy.log(`${options.index || ''}======Checking Available To Pay Trainer======`);
    cy.log(`======================================================`);
    cy.navTo('Trainers');
  ...
  const checkTrainerPayment = (options) => {
    ...
    cy.get('.ant-table-row > :nth-child(1)').contains(options.appointmentValues.client.LNF);
    ...
    if (options.appointmentValues.startTime) {
      cy
        .get('.ant-table-row > :nth-child(3)')
        .contains(options.appointmentValues.startTime);
    }
    if (options.appointmentValues2) {
      cy.get('.ant-table-row > :nth-child(1)').contains(options.appointmentValues2.client.LNF);
      cy.get('.ant-table-row > :nth-child(4)').contains(options.appointmentValues2.appointmentType).should('exist');
      cy
        .get('.ant-table-row > :nth-child(2)')
      ...
      if (options.appointmentValues2.startTime) {
        cy
          .get('.ant-table-row > :nth-child(3)')
          .contains(options.appointmentValues2.startTime);
      }
    }
  };
  ...
  const checkVerification = (options) => {
    ...
    if (options.inarrearsItemValues) {
      cy.get('.row-in-arrears > :nth-child(2)').contains(options.inarrearsItemValues.client.LNF).should('exist');
      cy.get('.row-in-arrears > :nth-child(5)').contains(options.inarrearsItemValues.appointmentType).should('exist');
      cy
        .get('.row-in-arrears > :nth-child(3)')
        .contains(options.inarrearsItemValues.date.format('MM/DD/YYYY'))
        .should('exist');
      if (options.inarrearsItemValues.startTime) {
        cy
          .get('.row-in-arrears > :nth-child(4)')
          .contains(options.inarrearsItemValues.startTime);
      }
    }
    if (options.inarrearsItemValues2) {
      cy.get('.row-in-arrears > :nth-child(2)').contains(options.inarrearsItemValues2.client.LNF).should('exist');
      cy.get('.row-in-arrears > :nth-child(5)').contains(options.inarrearsItemValues2.appointmentType).should('exist');
      cy
        .get('.row-in-arrears > :nth-child(3)')
      ...
      if (options.inarrearsItemValues2.startTime) {
        cy
          .get('.row-in-arrears > :nth-child(4)')
          .contains(options.inarrearsItemValues2.startTime);
      }
    }
    ...
    if (options.noAvailable) {
      Cypress.$(`.ant-table-row`).each(row => {
        cy.wrap(row).find('.row-in-arrears').should('not.exist');
      });
    }
  };
  ...
  const clickOnAppointment = (options) => {
    cy.log(`======================================================`);
    cy.log(`${options.index || ''}======Click On Appointment: ${options.date.toString()}======`);
    cy.log(`======================================================`);

    navToAppropriateWeek(options.date);
    const appointment = cy
      .get(`ol[data-id='${options.date.format('ddd MM/DD')}']
 li[data-id='${options.time}'] .redux__task__calendar__task__item`);
  ...
  const createAppointment = (options) => {
    cy.log(`======================================================`);
    cy.log(`${options.index || ''}======Creating an Appointment: ${options.date.toString()}======`);
    cy.log(`======================================================`);

    navToAppropriateWeek(options.date);
    cy.get(`ol[data-id='${options.date.format('ddd MM/DD')}'] li[data-id='${options.time}']`)
      .click();
    ...
    if (options.trainer) {
      cy.get('#trainerId').focus({log: false}).click({force: true});
      cy
        .get('.ant-select-dropdown-menu-item')
    ...
    cy.get('form').submit();
    cy.wait(options.future ? '@scheduleAppointment' : '@scheduleAppointmentInPast');
 ...
 li[data-id='${options.time}'] div.redux__task__calendar__task__item`)
  ...
  const deleteAppointment = (options) => {
    cy.log(`======================================================`);
    cy.log(`${options.index || ''}======Delete Appointment: ${options.date.toString()}======`);
    cy.log(`======================================================`);
  ...
  const loginTrainer = (options) => {
    cy.log(`======================================================`);
    cy.log(`${options.index || ''}======Trainer Logging In: ${options.trainer.LNF}======`);
    cy.log(`======================================================`);
  ...
  const navToAppropriateWeek = (date) => {
    cy
      .get('.redux__task__calendar__week input[data-id="startOfWeek"]', { log: false })
      .invoke('val')
      .as('sowValue');
    cy.get('@sowValue',{ log: false })
      .then(sow => {
        const startOfWeek = Cypress.moment(sow).add(1, 'day').startOf('day');
        if (date.isBefore(startOfWeek)) {
          cy.log(`======navigate one week back======`);
          cy.get('.redux__task__calendar__header__date__nav > :nth-child(1)', { log: false }).click({ log: false });
          cy.wait('@fetchAppointments', { log: false });
        }
      });

    cy
      .get('.redux__task__calendar__week input[data-id="endOfWeek"]', { log: false })
      .invoke('val')
      .as('eowValue');
    cy.get('@eowValue', { log: false })
      .then(eow => {
        const endOfWeek = Cypress.moment(eow).add(1, 'day').endOf('day');
        if (date.isAfter(endOfWeek)) {
          cy.log(`======navigate one week forward======`);
          cy.get('.redux__task__calendar__header__date__nav > :nth-child(2)', { log: false }).click({ log: false });
          cy.wait('@fetchAppointments', { log: false });
        }
      });
  };
  ...
  const purchaseSessions = (options) => {
    cy.log(`======================================================`);
    cy.log(`${options.index || ''}======Purchasing Sessions for ${options.client.LNF} ======`);
    cy.log(`======================================================`);
