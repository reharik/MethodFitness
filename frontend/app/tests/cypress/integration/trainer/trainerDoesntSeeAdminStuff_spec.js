...
describe('Trainer Can Not See Admin Stuff', () => {
  ...
  describe('landing on calendar page', () => {
    it('should not be able to see or do admin stuff', function() {
      ...
      cy.get(`ol[data-id='${aDT.date.format('ddd MM/DD')}'] li[data-id='${aDT.time}']`)
        .click();
      ...
      cy
        .get('.ant-select-dropdown-menu-item')
        .contains(this.clients.client2.LNF).should('not.exist');
      cy
        .get('.ant-select-dropdown-menu-item')
        .contains(this.clients.client5.LNF).should('not.exist');
      cy.get('#clients').blur();
      ...
      cy.get(`[title="${newDate.format('MM/DD/YYYY')}"].ant-calendar-disabled-cell`).should('exist');
      ...
      cy
        .get(`ol[data-id='${aDT.date.format('ddd MM/DD')}']
 li[data-id='${aDT.time}'] .redux__task__calendar__tasks`)
        .click();

      cy.get('div.ant-confirm-body span').contains('You can not set an appointment in the past');
  ...
  describe('when trainer is in app', () => {
    it('should see the right stuff in menu', function() {
      ...
      routines.loginTrainer({ index: 1, trainer });
      cy.visit('/');
      cy.get('.menu__item__leaf__link').contains('Trainers').should('not.exist');
  ...
  describe.only('when trainer is in clients view', () => {
    it('should see the right stuff in client list', function() {
      ...
      cy
        .get('.ant-table-row').should('have.lengthOf', 3);
      cy
        .get('.ant-table-row')
        .find('span')
  ...
  it('should see the right stuff in list', function() {
    ...
    routines.loginTrainer({ index: 1, trainer });
    cy.visit('/');
    routines.purchaseSessions({index: 1, client: this.clients.client3});
    cy.get('#purchaseList').contains('Submit Refund').should('not.exist');
  });
});
});
