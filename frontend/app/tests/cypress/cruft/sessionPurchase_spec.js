...
describe.skip('Session Purchase', () => {
  ...
  describe.only('When entering new items to purchase', () => {
    it('Should add up the total correctly and submit successfully', function() {
      ...
      cy.dataId('purchaseTotal', 'div')
        .find('div.ant-card-head-title')
        .contains(this.prices.fullHour * 2);
      ...
      cy.dataId('fullHourTenPack-container', 'div')
        .next('div')
        .contains(this.prices.fullHourTenPack * 2);
      cy.dataId('purchaseTotal', 'div')
        .find('div.ant-card-head-title')
        .contains((this.prices.fullHour + this.prices.fullHourTenPack) * 2);
      cy.log('-----HALF_HOUR-----');
      ...
      cy.dataId('halfHour-container', 'div')
        .next('div')
        .contains(this.prices.halfHour * 2);
      cy.dataId('purchaseTotal', 'div')
        .find('div.ant-card-head-title')
        .contains(
          (this.prices.fullHour +
      ...
      cy.dataId('halfHourTenPack-container', 'div')
        .next('div')
        .contains(this.prices.halfHourTenPack * 2);
      cy.dataId('purchaseTotal', 'div')
        .find('div.ant-card-head-title')
        .contains(
          (this.prices.fullHour +
      ...
      cy.dataId('pairTenPack-container', 'div')
        .next('div')
        .contains(this.prices.pairTenPack * 2);
      ...
      cy.get('div.ant-confirm-content').contains(
        `66 Sessions for $${total}.00`,
      );
      cy.get('button')
        .contains('OK')
        .click({ force: true });
  ...
  describe('When making purchase after unfunded appointment', () => {
    it('Should set one session as used for unfunded appointment', function() {
      ...
      cy.wait(2000);
      cy.get('tr.ant-table-row-level-0')
        .find('td')
        .contains(this.prices.fullHour * 2)
        .closest('tr')
        .find('.ant-table-row-expand-icon')
